...
def pretty_print(data, output_func=print):
    from tally_pipes.pipes.pretty_print import pretty_print_not_iterative
...
def print_results(
  ...
  if "json" == output_format:
    output_func(
      ...
      from tally_pipes.pipes.pdf import pdf_print
      try:
...
def func_path_sanity_checks(func_path, pipes_whitelist):
    ...
    if " " in func_path or len(values) == 0 or len(values) > 4 or\
        values[0] != "tally_pipes" or values[1] != "pipes":
        raise Exception()
